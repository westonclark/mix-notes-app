...
import cancel from '../assets/cancel_FILL0_wght400_GRAD0_opsz48 (1).png';
  ...
  return (
    <>
      <article className="song" style={{ height: showNotes ? 'auto' : '3rem' }}>
        <div
          className="song-top"
          style={{ height: showNotes ? '28px' : '100%' }}
          onClick={() => {
          ...
          <div>
            Notes
            <img src={!showNotes ? more : less}></img>
          </div>
        </div>
        {showNotes ? (
...
function Note({ note }) {
  ...
  return (
    <div className="note" key={note.id}>
      ...
      {/* <img src={cancel} alt="" onClick={deleteNote(note.id)} /> */}
    </div>
  );
}
